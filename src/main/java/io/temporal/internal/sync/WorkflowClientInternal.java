...
public final class WorkflowClientInternal implements WorkflowClient {
  ...
  public <T> T newWorkflowStub(
    ...
    WorkflowExecution.Builder execution = WorkflowExecution.newBuilder().setWorkflowId(workflowId);
    if (runId.isPresent()) {
      execution.setRunId(runId.get());
    }
    WorkflowInvocationHandler invocationHandler =
        new WorkflowInvocationHandler(
            workflowInterface, genericClient, execution.build(), dataConverter, interceptors);
    @SuppressWarnings("unchecked")
    T result =
        (T)
            Proxy.newProxyInstance(
  ...
  public WorkflowStub newUntypedWorkflowStub(
    ...
    return newUntypedWorkflowStub(execution.build(), workflowType);
  }
