...
class StateMachines {
  ...
  enum State {
    NONE,
    INITIATED,
    INITIATED_QUERY_ONLY, // Used for query only decisions
    STARTED,
    STARTED_QUERY_ONLY,
    FAILED,
    TIMED_OUT,
  ...
  static final class DecisionTaskData {
    ...
    void clear() {
      previousStartedEventId = NO_EVENT_ID;
                ...
                if (e.getStatus().getCode() == Status.Code.ALREADY_EXISTS) {
                  StartChildWorkflowExecutionFailedEventAttributes failRequest =
                      StartChildWorkflowExecutionFailedEventAttributes.newBuilder()
                          ...
                          .setCause(WorkflowExecutionFailedCause.WorkflowAlreadyRunning)
                          .build();
                  try {
  ...
  private static void scheduleQueryDecisionTask(
    ...
    ctx.onCommit(
        (historySize) -> {
          ...
          data.previousStartedEventId = NO_EVENT_ID;
        });
  }
