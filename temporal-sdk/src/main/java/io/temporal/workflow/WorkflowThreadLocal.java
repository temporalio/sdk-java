...
public final class WorkflowThreadLocal<T> {
   ...
   * #set(Object)} has not yet been called in the thread. Note that the value returned by the {@code
   * Supplier} is not stored in the {@code WorkflowThreadLocal} implicitly; repeatedly calling
   * {@link #get()} will always re-execute the {@code Supplier} until you call {@link #set(Object)}
   * for the first time. This differs from the behavior of {@code ThreadLocal}. If you want the
   * value returned by the {@code Supplier} to be stored in the {@code WorkflowThreadLocal}, which
   * matches the behavior of {@code ThreadLocal}, use {@link #withCachedInitial(Supplier)} instead.
   *
   * @param supplier Callback that will be executed whenever {@link #get()} is called, until {@link
   *     #set(Object)} is called for the first time.
  ...
  /**
   * Create an instance that returns the value returned by the given {@code Supplier} when {@link
   * #set(Object)} has not yet been called in the Workflow, and then stores the returned value
   * inside the {@code WorkflowThreadLocal}.
   *
   * @param supplier Callback that will be executed when {@link #get()} is called for the first
   *     time, if {@link #set(Object)} has not already been called.
   * @return A {@code WorkflowThreadLocal} instance.
   * @param <S> The type stored in the {@code WorkflowThreadLocal}.
   */
