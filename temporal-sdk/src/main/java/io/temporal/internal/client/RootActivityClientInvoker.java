...
public class RootActivityClientInvoker implements ActivityClientCallsInterceptor {
  ...
  public StartActivityOutput startActivity(StartActivityInput input) {
    ...
    if (nexusContext != null) {
      // Activity starts made on this operation-handler thread get the inbound links and
      // request-ID deduplication, including starts through a raw ActivityClient. This is
      // deliberately broader than nexusOperationMetadata: links and deduplication do not carry
      // completion semantics, unlike a completion callback.
