...
class HeartbeatContextImpl implements HeartbeatContext {
   ...
   */
  private RecordActivityTaskHeartbeatRequest offloadHeartbeat(
    ...
    try {
      return externalStorage.storeBlocking(request, activityStorageTarget(), offloadCancel.token());
    } finally {
  ...
  private void sendHeartbeatRequest(Object details) {
    try {
      ...
      if (externalStorage != null) {
        request = offloadHeartbeat(request);
      }
