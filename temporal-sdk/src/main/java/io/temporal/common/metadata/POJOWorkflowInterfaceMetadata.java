...
public final class POJOWorkflowInterfaceMetadata {
  ...
  private Set<POJOWorkflowMethod> getWorkflowInterfaceMethods(
    ...
    return getWorkflowInterfaceMethods(current, current, forceProcessWorkflowMethods, dedupeMap);
  }

  /**
   * @param root the first interface on the hierarchy, bottom-up ({@code current} or an interface
   *     that extends it)
  ...
  private Set<POJOWorkflowMethod> getWorkflowInterfaceMethods(
    ...
    for (Class<?> anInterface : interfaces) {
      Set<POJOWorkflowMethod> parentMethods =
          getWorkflowInterfaceMethods(root, anInterface, false, dedupeMap);
      addParentMethods(parentMethods, root, current, result);
    }
    ...
    for (Method declaredMethod : declaredMethods) {
      ...
      if (validateAndQualifiedForWorkflowMethod(methodMetadata)) {
        POJOWorkflowMethod workflowMethod = withGenericTypesResolved(methodMetadata, current);
        result.add(workflowMethod);
      }
    }
  ...
  private static void addParentMethods(
    ...
    for (POJOWorkflowMethod parentMethod : parentMethods) {
      if (parentMethod.getType() == WorkflowMethodType.NONE) {
        ...
        // adding to the
        // result
        POJOWorkflowMethod workflowMethod = withGenericTypesResolved(parentMethod, rootInterface);
        toSet.add(workflowMethod);
      }
    }
  }

  private static POJOWorkflowMethod withGenericTypesResolved(
      POJOWorkflowMethod workflowMethod, Class<?> anInterface) {
    return workflowMethod.withGenericTypes(
        POJOReflectionUtils.getGenericParameterTypes(workflowMethod.getMethod(), anInterface),
        POJOReflectionUtils.getGenericReturnType(workflowMethod.getMethod(), anInterface));
  }
  ...
  private static boolean validateAndQualifiedForWorkflowMethod(POJOWorkflowMethod workflowMethod) {
    ...
    if (isAnnotatedWorkflowMethod) {
      ...
      // except if they are bridge methods generated by the java compiler, because
      // the compiler also copies the annotations (JDK-6695379).
      // https://github.com/temporalio/sdk-java/issues/1107
      if (method.isBridge()) {
        return false;
      }
      return true;
