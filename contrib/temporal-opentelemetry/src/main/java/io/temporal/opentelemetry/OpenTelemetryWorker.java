...
public final class OpenTelemetryWorker {
  ...
  /**
   * Configures metrics, tracing interceptors, and flushing for a Temporal worker.
   *
   * <p>By default this method creates an OpenTelemetry SDK with OTLP trace and metric exporters. If
   * {@link Builder#setOpenTelemetry(OpenTelemetry)} is used, the provided instance is used instead
  ...
  /**
   * Registers an OpenTelemetry force-flush hook.
   *
  ...
  public static final class Builder {
    ...
    public void apply(
      ...
      configureMetrics(
          serviceStubsOptions,
          addShutdownHook,
          resolvedOpenTelemetry,
          getServiceName(),
          metricsReportInterval);
      configureTracing(clientOptions, workerFactoryOptions, resolvedOpenTelemetry);
      if (flushHook == null) {
        configureFlushHook(addShutdownHook, resolvedOpenTelemetry, flushTimeout);
      } else {
        Objects.requireNonNull(addShutdownHook, "addShutdownHook").accept(flushHook);
      }
    }
  }
