...
public class S3StorageDriverTest {
  ...
  private static S3StorageDriver driver(S3StorageDriverClient client) {
  ...
  public void builderDefaults() {
    S3StorageDriver driver = driver(new InMemoryS3StorageDriverClient());
  ...
  public void builderCustomName() {
    S3StorageDriver driver =
        S3StorageDriver.newBuilder()
            .setClient(new InMemoryS3StorageDriverClient())
            .setBucket("b")
            .setName("custom-name")
            .build();
  ...
  public void builderRequiresBucket() {
    S3StorageDriver.newBuilder().setClient(new InMemoryS3StorageDriverClient()).build();
  }

  @Test(expected = IllegalArgumentException.class)
  public void builderRejectsNonPositiveMaxPayloadSize() {
    S3StorageDriver.newBuilder().setMaxPayloadSize(0);
  }
  ...
  public void storeDeduplicatesIdenticalPayloads() {
    InMemoryS3StorageDriverClient client = new InMemoryS3StorageDriverClient();
  ...
  public void storeKeyPercentEncodesSpecialChars() {
    ...
    assertTrue(
        key,
        key.startsWith(
            "v0/ns/my%20namespace/wt/my%2Fworkflow/wi/wf%20id%2B1/ri/run%3Dabc/d/sha256/"));
  }
  ...
  public void storageKeyEscapesPathSegmentsByContract() {
    ...
    assertEquals(
        "azAZ09-_.%7E%24%26%2B%3A%3D%40", S3StorageKey.escapePathSegment("azAZ09-_.~$&+:=@"));
    assertEquals(
  ...
  public void storageKeyReadmeExamples() {
    ...
    assertEquals("wf%20id%2B1", S3StorageKey.escapePathSegment("wf id+1"));
    assertEquals("attempt%3D1", S3StorageKey.escapePathSegment("attempt=1"));
    ...
    assertEquals(
        "v0/ns/payments%20prod/wt/ChargeWorkflow/wi/order%2B123%3Dabc/ri/3f1d6c7a-8b2e-4f7a-9d0a-87a6f95e4d31/d/sha256/"
            + workflowDigest,
    ...
    assertEquals(
        "v0/ns/payments%20prod/at/Capture%2FCharge/ai/activity%20id%2B42/ri/9e1d1fd9-2f8a-4c40-93e2-731f31b9268b/d/sha256/"
            + activityDigest,
        S3StorageKey.forPayload(
   ...
   */
  private static final class HoldSecondUploadClient implements S3StorageDriverClient {
  ...
  /** In-memory {@link S3StorageDriverClient} with optional error injection, for unit tests. */
  private static final class InMemoryS3StorageDriverClient implements S3StorageDriverClient {
