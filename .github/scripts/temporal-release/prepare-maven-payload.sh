...
build_and_sign() {
  ...
  mkdir -p "$sandbox/gradle" "$sandbox/home" "$sandbox/source"
  mkdir "$gnupg"
  chmod 0700 "$gnupg"
  ...
  [[ $(grep -Ec "id ['\"]io\.github\.gradle-nexus\.publish-plugin['\"] version" \
      "$sandbox/source/build.gradle") == 1 &&
    $(grep -Ec "id ['\"]io\.github\.gradle-nexus\.publish-plugin['\"] version ['\"]1\.3\.0['\"]" \
      "$sandbox/source/build.gradle") == 1 ]] ||
    conflict "sdk-java must use the supported Gradle Nexus plugin 1.3.0."
  docker run --rm --pull=missing --network bridge --cap-drop ALL \
    ...
    --mount "type=bind,src=$generated,dst=/payload" \
  ...
  find "$generated/io/temporal" -type f \
  ...
  while IFS= read -r -d '' payload; do
    gpg --batch --yes --homedir "$gnupg" --pinentry-mode loopback \
      ...
      fail "trusted signing failed for ${payload#"$generated/"}."
    md5sum "$payload" | awk '{print $1}' >"$payload.md5"
    sha1sum "$payload" | awk '{print $1}' >"$payload.sha1"
  done < <(find "$generated/io/temporal" -type f \
